...
async function run() {
  console.log('🚀 이수페타시스 (007660) 실시간 데이터 동기화 및 심층 분석 시작...');
  
  // 1. Sync live data from KIS
  await syncSingleStock('007660', false);
  ...
  // 3. Fetch News
  const newsRes = await fetchNaverNews('이수페타시스 주식 전망 공시 뉴스');
  ...
  종목 [이수페타시스 (007660)]에 대해 제공된 계량 지표와 최신 뉴스, 그리고 계산된 퀀트 점수 및 리스크 필터를 종합하여 투자 의견을 도출해줘.
