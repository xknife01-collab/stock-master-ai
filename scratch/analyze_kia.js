...
async function run() {
  console.log('🚀 기아 (000270) 실시간 데이터 동기화 및 심층 분석 시작...');
  
  // 1. Sync live data from KIS
  await syncSingleStock('000270', false, true);
  ...
  // 3. Fetch News
  const newsRes = await fetchNaverNews('기아 주식 전망 공시 뉴스');
  ...
  if (!isFinancialSector && debtRatio > 200) vetoReasons.push(`고부채 위험 (${debtRatio}%)`); // Standard threshold is 200% for non-financials
  ...
  console.log(`- 현재가: ${data.fundamental?.price}원`);
  ...
  종목 [기아 (000270)]에 대해 제공된 계량 지표와 최신 뉴스, 그리고 계산된 퀀트 점수 및 리스크 필터를 종합하여 투자 의견을 도출해줘.
