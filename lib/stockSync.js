        ...
        while (true) {
            try {
                ...
                for (let i = 0; i < targetSymbols.length; i++) {
                    // 📢 [우선순위 큐 처리] 실시간 검색/추천으로 유입된 동적 대기열(syncQueue) 최우선 처리
                    while (syncQueue.size > 0) {
                        const priSymbol = syncQueue.values().next().value;
                        if (priSymbol) {
                            console.log(`🔥 [Sync Engine Priority] Processing high-priority on-demand sync for: ${priSymbol}`);
                            try {
                                await syncSingleStock(priSymbol);
                                syncedThisCycle.add(priSymbol);
                            } catch (err) {
                                console.error(`❌ [Sync Engine Priority] Error syncing ${priSymbol}:`, err.message);
                            }
                            syncQueue.delete(priSymbol);
                            await delay(1500); // KIS API 스로틀링 유지
                        }
                    }

                    const symbol = targetSymbols[i];
                    if (syncedThisCycle.has(symbol)) continue; // 이미 우선순위 처리된 종목은 스킵
                    
                    try {
                        await syncSingleStock(symbol);
                        syncedThisCycle.add(symbol);
                    } catch (err) {
