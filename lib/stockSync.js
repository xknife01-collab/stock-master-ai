...
import { fetchStockFullDetailFromKIS, fetchStockChartFromKIS, fetchMarketRankings, fetchConditionResult, fetchInvestorNetBuyRankings } from './kisCore.js';
    ...
    try {
        ...
        try {
            ...
            if (data) {
                ...
                if (data.updated_at && data.fundamental?.finance?.length > 0 && data.advanced?.chartHistory?.['1Y']?.length > 0) {
                    ...
                    if (krToday === lastUpdatedDate) {
                        ...
                        // ⚙️ [Market Close Override] 장이 마감된 시간(15시 40분 이후)이고, 기존 캐시가 장중 실시간 가집계 데이터(isRealtime: true)라면,
                        // 최종 확정 수급 데이터(Daily Settlement)를 반영하기 위해 Fast Sync를 강제로 해제하고 전체 동기화를 실행합니다.
                        const krNow = new Date(Date.now() + 9 * 60 * 60 * 1000);
                        const curHHMM = krNow.getUTCHours().toString().padStart(2, '0') + krNow.getUTCMinutes().toString().padStart(2, '0');
                        if (curHHMM >= '1540' && data.advanced?.investor?.isRealtime === true) {
                            console.log(`🔄 [Sync Engine] Market is closed (${curHHMM} KST) and cached investor data is real-time estimate. Disabling FastSync to fetch settled daily trend for ${symbol}.`);
                            isFastSync = false;
                        }
                    }
                }
        ...
        console.log(`🔄 [Sync Engine] Fetching detail from KIS for: ${symbol} (FastSync: ${isFastSync})`);
        const result = await fetchStockFullDetailFromKIS(symbol, existing, isFastSync);
        ...
        let chart1D = [];
        try {
            chart1D = await fetchStockChartFromKIS(symbol, '1D');
        } catch (chartErr) {
            console.warn(`⚠️ [Sync Engine] Failed to fetch 1D chart for ${symbol}:`, chartErr.message);
        }
            ...
            try {
                ...
                while (syncQueue.size > 0) {
                    ...
                    if (priSymbol) {
                        ...
                        try {
                            await syncSingleStock(priSymbol);
                        } catch (err) {
                ...
                if (tier1Symbols.length > 0) {
                    ...
                    for (let i = 0; i < tier1Symbols.length; i++) {
                        ...
                        // 장중엔 1.5초 대기, 장외엔 5초 대기
                        await delay(marketOpen ? 1500 : 5000);
                    }
                }

                // 장중엔 2분 대기, 장외엔 15분 대기 후 다음 사이클 작동
                const sleepMs = marketOpen ? (2 * 60 * 1000) : (15 * 60 * 1000);
        ...
        while (true) {
            try {
                ...
                if (tier2Symbols.length > 0) {
                    ...
                    for (let i = 0; i < tier2Symbols.length; i++) {
                        const symbol = tier2Symbols[i];
                        
                        // 저속 루프 중에도 실시간 장중/장외 속도 조절
                        // 장중엔 15초 대기 (QPS 점유 방지), 장외엔 5초 대기
                        const loopDelay = marketOpen ? 15000 : 5000;
                ...
                if (curHour >= 16 && lastDailyChartSyncDate !== todayStr) {
                    ...
                    for (let j = 0; j < allSymbols.length; j++) {
                        ...
                        try {
                            ...
                            const chart1W = await fetchStockChartFromKIS(symbol, '1W');
                            await delay(300);
                            const chart1M = await fetchStockChartFromKIS(symbol, '1M');
                            await delay(300);
                            const chart1Y = await fetchStockChartFromKIS(symbol, '1Y');
