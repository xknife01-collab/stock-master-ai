 ...
 */
export async function syncSingleStock(symbol, isBackground = false) {
    ...
    try {
        ...
        console.log(`🔄 [Sync Engine] Fetching detail from KIS for: ${symbol} (FastSync: ${isFastSync}, Background: ${isBackground})`);
        const result = await fetchStockFullDetailFromKIS(symbol, existing, isFastSync, isBackground);
        ...
        let chart1D = [];
        try {
            chart1D = await fetchStockChartFromKIS(symbol, '1D', isBackground);
        } catch (chartErr) {
