...
// 동적 TPS 계산: 실전투자 키당 3 TPS (초당 거래건수 초과 EGW00201 방지), 모의투자 키당 2 TPS
export const kisRateLimiters = KIS_KEYS.length > 0
    ? KIS_KEYS.map(() => new KisRateLimiter(isVirtual ? 2 : 3))
    : [new KisRateLimiter(2)];

// 하위 호환성을 위한 기본 rate limiter 익스포트
export const kisRateLimiter = kisRateLimiters[0];
...
export const kisRequest = async (config, retries = 3, delayMs = 2500) => {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            // 키별 Rate Limiter를 통해 호출 실행
            const limiter = kisRateLimiters[keyIndex] || kisRateLimiters[0];
            const response = await limiter.enqueue(() => axios(config));
            const data = response.data;
            
            const rtVal = data && data.rt_cd !== undefined && data.rt_cd !== null ? String(data.rt_cd) : '';
            const msg = data?.msg1 || '';
            const msgCd = data?.msg_cd || '';
            const isOk = rtVal === '0' || msgCd === 'MCA05762' || msgCd === 'MCA05761';
            if (data && rtVal && !isOk) {
                
                const isRateLimit = msgCd === 'EGW00133' || 
                                    msgCd === 'EGW00201' ||
                                    msg.includes('초당 거래건수') ||
                                    msg.includes('초과') || 
                                    msg.includes('제한') || 
                                    msg.includes('TPS') || 
                                    msg.includes('과부하');
                ...
                if (isRateLimit) {
                    if (attempt < retries) {
                        ...
                        console.warn(`⚠️ [KIS API Rate Limit] Attempt ${attempt}/${retries} failed. Msg: ${msg}. Retrying in ${delayMs + 1000}ms...`);
            ...
            console.error(`🚨 [KIS API Debug] Request Failed: URL=${config.url}, Params=${JSON.stringify(config.params || {})}, Status=${status}, Data=${JSON.stringify(data || {})}, Error=${error.message}`);
            ...
            if (isTokenExpired) {
                ...
                if (!isTokenRequest && keyInfo) {
                    try {
                        ...
                        const nextLimiter = kisRateLimiters[keyIndex] || kisRateLimiters[0];
                        return await nextLimiter.enqueue(() => axios(config));
                    } catch (refreshErr) {
            ...
            const isRateLimit = msgCd === 'EGW00201' || 
                                msg1.includes('초당 거래건수') || 
                                msgCd === 'EGW00133' || 
                                msg1.includes('초과') || 
                                msg1.includes('제한') || 
                                msg1.includes('TPS') ||
            ...
            if ((isRetryable || isRateLimit) && attempt < retries) {
                // Failover: 네트워크/서버 에러 발생 시도 다른 키로 즉시 스위칭하여 재시도
                if (!isTokenRequest && KIS_KEYS.length > 1) {
                    keyIndex = (keyIndex + 1) % KIS_KEYS.length;
                    keyInfo = KIS_KEYS[keyIndex];
                    console.warn(`🔄 [KIS Failover] Network/RateLimit error. Switching key to Index ${keyIndex} for retry.`);
                }
                const sleepDuration = isRateLimit ? (delayMs + 1000 + (attempt * 1000)) : (delayMs + (attempt * 1000));
                console.warn(`⚠️ [KIS Network/RateLimit Error] Attempt ${attempt}/${retries} failed (${error.message || error.code || 'Unknown error'}). Retrying in ${sleepDuration}ms...`);
...
const kisRequestDirectForToken = async (keyIndex) => {
    ...
    const limiter = kisRateLimiters[keyIndex] || kisRateLimiters[0];
    return await limiter.enqueue(() => axios({
        method: 'post',
...
export const fetchStockChartFromKIS = async (symbol, range, isBackground = false, targetHour = '') => {
    ...
    const params = isIntraday ? {
        ...
        FID_INPUT_DATE_2: new Date(Date.now() + 9 * 60 * 60 * 1000).toISOString().slice(0,10).replace(/-/g,''),
        FID_PERIOD_DIV_CODE: 'D',
        FID_ORG_ADJ_PRC: '0'
    };
    ...
    try {
        ...
        if (response.data.rt_cd === '0') {
            ...
            let finalHistory = output2.reverse().map(item => {
                ...
                // 장중(09:00~15:30)일 때만 미래 시점 데이터 제외
                if (isIntraday && isTodayTradingActive && timeStr > currentTimeStr) return null;
            ...
            if (isIntraday && isTodayTradingActive) {
