...
export const fetchStockInvestorTrend = async (symbol, isBackground = false) => {
    try {
        ...
        const isTodayData = data[0] && data[0].stck_bsop_date === todayStr;
 ...
 */
export const fetchStockIntradayInvestorEstimate = async (symbol, isBackground = false) => {
    try {
        if (!intradayEstimatesCache || Date.now() - lastIntradayFetchTime > INTRADAY_CACHE_TTL) {
            ...
            await delay(isBackground ? 50 : 300);
 ...
 */
export const fetchStockFullDetailFromKIS = async (symbol, existing = null, isFastSync = false, isBackground = false) => {
    if (!existing) {
        isFastSync = false;
    } else {
    ...
    try {
        ...
        try {
            intradayRes = await fetchStockIntradayInvestorEstimate(symbol, isBackground);
        } catch (err) {
        ...
        if (investorStats) {
            if (investorStats.isTodayData) {
                // 오늘 정식 마감 데이터가 이미 수집된 경우
                isRealtime = false;
            } else if (!isMarketClosed) {
                // 장중인 경우: 실시간 가집계 또는 외국계 회원사 순매수 동향(폴백) 반영
                if (intradayRes) {
