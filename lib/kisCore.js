...
export const fetchStockAnalytics = async (symbol) => {
    try {
        ...
        // 실시간 체결강도 추출 (inquire-ccnl 의 output 첫번째 체결 정보)
        const strength = ccnlVal?.data?.output?.[0]?.tday_rltv || '-';

        // 공매도 거래 비중 추출 (daily-short-sale 의 output2 첫번째 일자 정보)
...
export const fetchStockInvestorTrend = async (symbol) => {
    try {
        ...
        // 최근 3거래일치 단순 요약 (오타 수정: pru_ntby_qty -> prvs_ntby_qty)
        const rawSummary = data.slice(0, 3).map(it => 
            `${it.stck_bsop_date}: [외인:${it.frgn_ntby_qty}, 기관:${it.orgn_ntby_qty}, 개인:${it.prvs_ntby_qty}]`
        ).join(' | ');
        ...
        for (let i = 0; i < Math.min(5, data.length); i++) {
            ...
            personal5D += parseInt(data[i].prvs_ntby_qty) || 0;
        }

        // 2. 20일 누적 수급 계산 (개인 추가)
        let foreign20D = 0;
        let organ20D = 0;
        let personal20D = 0;
        for (let i = 0; i < Math.min(20, data.length); i++) {
            ...
            personal20D += parseInt(data[i].prvs_ntby_qty) || 0;
        }

        // 3. 연속 순매수 일수 계산 (개인 추가)
        ...
        for (let i = 0; i < data.length; i++) {
            const qty = parseInt(data[i].prvs_ntby_qty) || 0;
            if (qty > 0) personalConsecutiveDays++;
            else break;
        }
...
/**
 * 특정 종목의 핵심 기술적 퀀트 데이터 조회 (1회당 3개 API 호출)
 */
export const fetchStockQuantMetrics = async (symbol) => {
    try {
        ...
        const [technicals, ccnlRes, shortRes] = await Promise.allSettled([
            // 1. 최근 60거래일 일자별 시세 (이격도 계산용)
            axios.get(`${KIS_BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-daily-itemchartprice`, {
                params: {
                    FID_COND_MRKT_DIV_CODE: 'J', FID_INPUT_ISCD: symbol,
        ...
        const currentPrice = dailyPrices[0] ? parseInt(dailyPrices[0].stck_clpr || '0') : 0;
...
/**
 * 30개 후보 종목 리스트의 지표들을 딜레이(160ms)를 주며 안전하게 일괄 수집
 */
...
export const fetchIndexDailyHistory = async (symbol) => {
    try {
        ...
        const response = await axios.get(`${KIS_BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-daily-indexchartprice`, {
            params: {
        ...
        if (response.data && response.data.rt_cd === '0') {
            ...
            // 최신일순으로 들어오므로 reverse하여 과거 -> 최신순으로 정렬
