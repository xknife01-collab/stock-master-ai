...
export const fetchStockInvestorTrend = async (symbol) => {
    try {
        ...
        const foreign1D = data[0] ? (parseInt(data[0].frgn_ntby_qty) || 0) : 0;
        const organ1D = data[0] ? (parseInt(data[0].orgn_ntby_qty) || 0) : 0;
        const personal1D = data[0] ? (parseInt(data[0].prsn_ntby_qty) || 0) : 0;
...
export const fetchConditionResult = async (seq, userId = process.env.KIS_USER_ID || 'dummy_id') => {
    try {
        ...
        const response = await kisRequest({
            method: 'get',
            url: `${KIS_BASE_URL}/uapi/domestic-stock/v1/quotations/condition-search`,
            params: {
                user_id: userId,
                seq: seq
            },
...
export const fetchStockIntradayInvestorEstimate = async (symbol) => {
    try {
        ...
        const res = await kisRequest({
            method: 'get',
            url: `${KIS_BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-investor-trend-estimate`,
            params: {
                FID_COND_MRKT_DIV_CODE: 'J',
                FID_INPUT_ISCD: symbol
            },
            headers: { ...commonHeaders, 'tr_id': 'HHKSB39012000' }
        });

        if (res.data && res.data.output) {
            const out = res.data.output;
            const foreign = parseInt(out.frgn_ntby_qty) || 0;
            const organ = parseInt(out.orgn_ntby_qty) || 0;
            const personal = parseInt(out.prsn_ntby_qty) || 0;
            return { foreign, organ, personal };
        }
        return null;
    } catch (e) {
