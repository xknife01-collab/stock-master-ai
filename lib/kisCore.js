...
export const fetchStockFullDetailFromKIS = async (symbol, existing = null, isFastSync = false) => {
    ...
    try {
        ...
        try {
            ...
            console.warn(`[Non-critical Check Failed] price failed for ${symbol}:`, err.message);
        }
        await delay(700);
        ...
        if (!isFastSync) {
            try {
                ...
                console.warn(`[Non-critical Check Failed] ratio failed for ${symbol}:`, err.message);
            }
            await delay(700);
        }
        ...
        try {
            ...
            console.warn(`[Non-critical Check Failed] ccnl failed for ${symbol}:`, err.message);
        }
        await delay(700);
        ...
        if (!isFastSync) {
            try {
                ...
                console.warn(`[Non-critical Check Failed] short failed for ${symbol}:`, err.message);
            }
            await delay(700);
        }
        ...
        if (!isFastSync) {
            try {
                ...
                console.warn(`[Non-critical Check Failed] credit failed for ${symbol}:`, err.message);
            }
            await delay(700);
        }
        ...
        if (!isFastSync) {
            try {
                ...
                console.warn(`[Non-critical Check Failed] daily failed for ${symbol}:`, err.message);
            }
            await delay(700);
        }
        ...
        if (!isFastSync) {
            try {
                ...
                console.warn(`[Non-critical Check Failed] investor failed for ${symbol}:`, err.message);
            }
            await delay(700);
        }
