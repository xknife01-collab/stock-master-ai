...
export const fetchStockFullDetailFromKIS = async (symbol) => {
    ...
    try {
        ...
        try {
            ...
            errors.push(`ratio: ${err.message}`);
        }
        await delay(300);

        // 3. consensus (Non-critical)
        try {
            ...
            console.warn(`[Non-critical Check Failed] consensus failed for ${symbol}:`, err.message);
        }
        await delay(300);

        // 4. income (Non-critical)
        try {
            ...
            console.warn(`[Non-critical Check Failed] income failed for ${symbol}:`, err.message);
        }
        await delay(300);
        ...
        try {
            ...
            errors.push(`credit: ${err.message}`);
        }
        await delay(300);
        ...
        try {
            ...
            errors.push(`investor: ${err.message}`);
        }
        await delay(300);

        // 10. intraday (Non-critical)
        try {
            intradayRes = await fetchStockIntradayInvestorEstimate(symbol);
        } catch (err) {
            console.warn(`[Non-critical Check Failed] intraday failed for ${symbol}:`, err.message);
        }
        ...
        const sector = priceRes?.data?.output?.bstp_kor_isnm ? priceRes.data.output.bstp_kor_isnm.trim() : '기타';
        ...
        if (shortRes?.data?.output2 && shortRes.data.output2.length > 0) {
            ...
            shortRatio = validRow?.ssts_vol_rlim !== undefined ? String(validRow.ssts_vol_rlim) : '0';
        }
        ...
        const advanced = {
            ...
            atr,
            atrPercent,
            transactionValue // 당일 누적 거래대금 (원 단위) 추가
        };
        ...
        if (isInvalidNumeric(advanced.transactionValue)) {
            throw new Error(`[Sanity Error] Invalid Transaction Value: ${advanced.transactionValue}`);
        }
